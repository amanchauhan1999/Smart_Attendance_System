...
export default function StudentDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`flex items-center gap-4 p-4 rounded-xl border ${isPresentToday ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-amber-500/10 border-amber-500/20'}`}>
        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isPresentToday ? 'bg-emerald-500/20' : 'bg-amber-500/20'}`}>
          {isPresentToday ? (
            ...
            <svg className="w-5 h-5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
          )}
        </div>
        <div>
          <p className={`font-semibold text-sm ${isPresentToday ? 'text-emerald-400' : 'text-amber-400'}`}>
      ...
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {logs.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-500">No attendance records yet</td></tr>
              ) : logs.map((log, i) => (
                <tr key={log.id} className="border-b border-slate-700/30 hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-3 text-sm text-slate-500 font-medium">{String(i + 1).padStart(2, '0')}</td>
                  <td className="px-6 py-3 text-sm text-white">{log.date || (log.timestamp ? new Date(log.timestamp).toLocaleDateString() : '-')}</td>
