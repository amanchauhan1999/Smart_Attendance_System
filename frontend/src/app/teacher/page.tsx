...
interface Log { id: number; student?: { name: string; rollNo: string }; status: string; timestamp: string; }

export default function TeacherDashboard() {
  const [stats, setStats] = useState({ totalStudents: 0, todayAttendance: 0, totalDaysActive: 0, todayPercentage: 0 });
  ...
  if (loading) return <div className="text-cyan-400 animate-pulse text-center mt-20">Loading dashboard...</div>;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatsCard label="Total Students" value={stats.totalStudents} icon="&#127891;" />
        <StatsCard label="Today Present" value={stats.todayAttendance} icon="&#9989;" />
        <StatsCard label="Active Days" value={stats.totalDaysActive} icon="&#128197;" />
        <StatsCard label="Today %" value={`${stats.todayPercentage?.toFixed(1) || 0}%`} icon="&#128200;" />
      </div>

      <Card className="border-cyan-500/20 bg-slate-950/60 backdrop-blur-sm">
        <CardHeader><CardTitle className="text-sm tracking-widest uppercase text-cyan-400">Recent Attendance</CardTitle></CardHeader>
        <CardContent>
          {logs.length === 0 ? (
