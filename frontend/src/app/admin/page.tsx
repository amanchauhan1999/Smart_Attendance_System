    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-800/50 rounded-xl border border-slate-700/50">
        <div className="px-6 py-4 border-b border-slate-700/50 flex items-center justify-between">
          ...
          <a href="/admin/attendance" className="text-indigo-400 text-sm hover:text-indigo-300 font-medium">View All</a>
        </div>
        <div className="px-6 py-3 border-b border-slate-700/50 flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg border border-slate-700/50 bg-slate-900/50 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500/50"
            />
          </div>
          <div className="text-sm text-slate-500">
            Sort by: <span className="text-slate-300 font-medium">Recent</span>
          </div>
        </div>
                ...
                paginated.map((log, i) => (
                  <tr key={log.id} className="border-b border-slate-700/30 hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-3 text-sm text-slate-500 font-medium">
                      {String((currentPage - 1) * perPage + i + 1).padStart(2, '0')}
                    </td>
                    ...
                    <td className="px-6 py-3 text-sm text-slate-400">
                      {log.date || (log.timestamp ? new Date(log.timestamp).toLocaleDateString() : '-')}
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-400">
        ...
        <div className="px-6 py-3 border-t border-slate-700/50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-sm border border-slate-700/50 text-slate-400 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Prev
            </button>
